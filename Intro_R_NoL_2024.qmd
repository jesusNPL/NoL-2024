...
-   R ([Version 4.4+](https://cran.r-project.org)) and RStudio ([Version 2+764](https://posit.co/download/rstudio-desktop/))
...
::: callout-tip
When using the **setwd()** function, we need to use quotation marks to specify the correct address. For example: **setwd("Dropbox/Teaching/NOL_Itasca/NoL-2024")**.
:::
...
#| echo: true
#| eval: false
dat <- na.omit(read.csv(file.choose()))
```
...
oak_names <- unique(dat$Species)

for (i in 1:length(oak_names)){ 
  
  print(paste0("Species ", 
               oak_names[i], # vector with species names
               ", was used in the Intro to R 2024!")
        )
...
## Read data
oaks <- read_csv("data/live.oaks.csv")
...
If you don't have an specific R package installed you just need to run the function install.packages("package name")
